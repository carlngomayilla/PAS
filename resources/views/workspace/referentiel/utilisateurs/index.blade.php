...
@section('content')
    ...
    <div class="app-screen-flow">
        ...
        <section class="app-screen-block" aria-labelledby="users-list-title">
            ...
            <form method="GET" action="{{ route('workspace.referentiel.utilisateurs.index') }}" class="mt-4 grid gap-3 rounded-lg border border-slate-200 bg-slate-50/85 p-3 md:grid-cols-2 xl:grid-cols-6 dark:border-slate-700 dark:bg-slate-900/70" data-auto-filter-form>
                                    ...
                                    <td>
                                        <div class="flex min-w-[190px] flex-wrap items-start gap-2">
                                            ...
                                            @elseif (($canRequestUserDeletion ?? false) && (int) auth()->id() !== (int) $row->id)
                                                <details class="relative" data-preview-ignore="1"><summary class="btn btn-danger btn-sm cursor-pointer list-none" data-preview-ignore="1">Demander suppression</summary><form method="POST" action="{{ route('workspace.referentiel.utilisateurs.deletion-requests.store', $row) }}" class="absolute right-0 z-20 mt-2 grid w-72 gap-2 rounded-lg border border-slate-200 bg-white p-3 shadow-xl dark:border-slate-700 dark:bg-slate-900" data-preview-ignore="1">@csrf<label class="grid gap-1 text-xs font-bold uppercase text-slate-500" data-preview-ignore="1">Motif<input name="motif" type="text" minlength="5" maxlength="1000" required class="min-h-10 rounded-lg border border-slate-300 px-3 text-sm font-normal normal-case dark:border-slate-600 dark:bg-slate-950 dark:text-slate-100" data-preview-ignore="1"></label><button class="btn btn-danger btn-sm" type="submit" data-preview-ignore="1">Transmettre</button></form></details>
                                            @endif
                                        </div>
                                    </td>
                                @endif
