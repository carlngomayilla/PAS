...
@section('title', 'Indicateur de performance et statistiques')
...
@section('content')
    ...
    <section class="grid gap-3 [grid-template-columns:repeat(auto-fit,minmax(220px,1fr))] mb-3.5">
        <article class="ui-card !mb-0">
            <p class="text-sm text-slate-500">Indicateur de performance visibles</p>
            <p class="mt-2 text-3xl font-bold text-slate-900">{{ $summary['visible'] }}</p>
            <p class="mt-2 text-sm text-slate-600">Indicateurs exposes dans les synthèses consolidees.</p>
        </article>
        <article class="ui-card !mb-0">
    ...
    <section class="showcase-panel mb-4">
        <form method="POST" action="{{ route('workspace.super-admin.kpis.update') }}" class="form-shell">
            ...
            <div class="form-actions">
                <button class="btn btn-primary" type="submit">Enregistrer le registre Indicateur de performance</button>
            </div>
        </form>
    </section>
@endsection
