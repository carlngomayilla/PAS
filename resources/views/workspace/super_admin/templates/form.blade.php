        ...
        <form method="POST" class="form-shell" action="{{ $isEdit ? route('workspace.super-admin.templates.update', $template) : route('workspace.super-admin.templates.store') }}">
            ...
            <x-ui.collapsible-section title="Identité et profil destinataire" open>
                <div class="form-grid">
                    ...
                    <div><label for="target_profile">Profil destinataire</label><select id="target_profile" name="target_profile"><option value="">Tous profils</option>@foreach ($profileOptions as $option)<option value="{{ $option }}" @selected(old('target_profile', $template->target_profile) === $option)>{{ $profileLabels[$option] ?? $option }}</option>@endforeach</select></div>
            ...
            <x-ui.collapsible-section title="Métadonnées">
                <div class="form-grid">
            ...
            <x-ui.collapsible-section title="Mise en page et style">
                <div class="form-grid">
                    ...
                    <div><label for="header_text">En-tête</label><input id="header_text" name="header_text" type="text" value="{{ old('header_text', $layout['header_text'] ?? '') }}"></div>
                    ...
                    <div><label for="excel_detail_sheet_name">Feuille détail Excel</label><input id="excel_detail_sheet_name" name="excel_detail_sheet_name" type="text" maxlength="31" value="{{ old('excel_detail_sheet_name', $layout['excel_detail_sheet_name'] ?? 'Reporting') }}"></div>
                    ...
                    <div class="md:col-span-2 xl:col-span-4 flex flex-wrap items-end gap-3">
                        ...
                        <label class="checkbox-pill !mb-0"><input name="word_page_break_after_summary" type="checkbox" value="1" @checked(old('word_page_break_after_summary', $layout['word_page_break_after_summary'] ?? false))>Word saut après synthèse</label>
                    </div>
                </div>
            </x-ui.collapsible-section>

            <x-ui.collapsible-section title="Blocs exportables">
                <div class="flex flex-wrap gap-3">
                    ...
                    <label class="checkbox-pill !mb-0"><input name="include_summary" type="checkbox" value="1" @checked(old('include_summary', $blocks['include_summary'] ?? 1))>Synthèse</label>
                    <label class="checkbox-pill !mb-0"><input name="include_detail_table" type="checkbox" value="1" @checked(old('include_detail_table', $blocks['include_detail_table'] ?? 1))>Table détail</label>
            ...
            <x-ui.collapsible-section title="Contenu dynamique">
                <div class="form-grid">
                ...
                <div class="mt-4">
                    <div class="text-sm font-semibold uppercase tracking-[0.18em] text-slate-500">Variables supportées</div>
