...
@section('content')
    ...
    @if (is_array($result))
        ...
        <section class="grid gap-3 [grid-template-columns:repeat(auto-fit,minmax(220px,1fr))] mb-3.5">
            ...
            <article class="ui-card !mb-0"><p class="text-sm text-slate-500">Taux d'exécution consolidé</p><p class="mt-2 text-3xl font-bold text-slate-900">{{ number_format((float) $result['current']['official_completion_rate'], 0) }}% -> {{ number_format((float) $result['simulated']['official_completion_rate'], 0) }}%</p><p class="mt-2 text-sm text-slate-600">Delta {{ $result['impact']['official_completion_rate_delta'] >= 0 ? '+' : '' }}{{ number_format((float) $result['impact']['official_completion_rate_delta'], 0) }} pts</p></article>
            <article class="ui-card !mb-0"><p class="text-sm text-slate-500">Score moyen consolidé</p><p class="mt-2 text-3xl font-bold text-slate-900">{{ number_format((float) $result['current']['official_average_score'], 0) }} -> {{ number_format((float) $result['simulated']['official_average_score'], 0) }}</p><p class="mt-2 text-sm text-slate-600">Delta {{ $result['impact']['official_average_score_delta'] >= 0 ? '+' : '' }}{{ number_format((float) $result['impact']['official_average_score_delta'], 0) }}</p></article>
