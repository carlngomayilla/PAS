...
@if ($unlockTarget && $unlockService->isLocked($unlockTarget))
    <section class="showcase-panel mb-4 border border-amber-200 bg-amber-50 app-screen-block">
        ...
        <p class="mt-2 text-sm font-medium text-amber-900">
            Cet element est verrouille apres enregistrement. Une modification exige un contrôle SCIQ/Planification puis une décision DG.
        </p>
        @if ($canAskUnlock && $unlockRoute)
            <form method="POST" action="{{ $unlockRoute }}" class="mt-4 grid gap-3 md:max-w-2xl" data-confirm-message="Transmettre une demande de modification aux contrôleurs ?" data-confirm-tone="warning" data-confirm-label="Transmettre">
                @csrf
                ...
                <button class="btn btn-primary justify-self-start" type="submit">Demander modification</button>
            </form>
        @endif
    </section>
@endif
