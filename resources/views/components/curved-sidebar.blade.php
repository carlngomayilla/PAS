...
<aside class="eas-sidebar glass-sidebar app-sidebar w-full md:sticky md:top-0 md:h-screen md:w-[280px] md:min-w-[280px]">
    <div class="dealdeck-sidebar-panel flex h-full flex-col border-r border-[#e5e7eb] bg-white text-[#1c203d]">
            ...
            <div class="flex items-center">
                <div class="min-w-0 flex-1">
                    <div class="rounded-2xl bg-white px-4 py-3 shadow-sm">
                        <x-brand.logo variant="wordmark" class="block h-auto w-full max-w-[9rem]" />
                    </div>

                    <div class="mt-3 truncate text-[11px] font-semibold uppercase tracking-wide text-[#64748b]">
        ...
        {{-- Navigation --}}
        <nav class="flex-1 overflow-y-auto px-3 py-5" aria-label="Navigation latérale">
            ...
            <div class="mb-6 space-y-1">
                @foreach ($mainItems as $item)
                    <a
                        ...
                        class="dealdeck-sidebar-link {{ $item['active'] ? 'is-active' : '' }} flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-semibold transition-all duration-200"
                    >
                        <span class="dealdeck-sidebar-link-icon flex h-9 w-9 items-center justify-center rounded-xl">
                            {!! $svgs[$item['icon']] ?? $svgs['grid'] !!}
                        </span>
            ...
            {{-- Planification --}}
            <div class="mb-6 space-y-1">
                <div class="mb-2 px-4 text-[11px] font-bold uppercase tracking-wider text-[#64748b]">
                    Planification
                </div>
        ...
        <div class="border-t border-[#e5e7eb] px-4 py-4">
            <form method="POST" action="{{ route('logout') }}">
                ...
                <button
                    type="submit"
                    class="dealdeck-sidebar-logout flex w-full items-center justify-center rounded-xl border border-[#3996d3]/40 bg-white px-4 py-3 text-sm font-bold text-[#3996d3] transition hover:bg-[#eef6fc]"
                >
                    Déconnexion
                </button>
            </form>
