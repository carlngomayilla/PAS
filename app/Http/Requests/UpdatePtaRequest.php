...
class UpdatePtaRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $uniqueObjectif = Rule::unique('ptas', 'objectif_operationnel_id')
            ->where(fn ($query) => $query->whereNull('deleted_at'));

        $currentId = $this->resolveCurrentPtaId();
        if ($currentId !== null) {
            $uniqueObjectif = $uniqueObjectif->ignore($currentId);
        }

        return [
            'objectif_operationnel_id' => ['required', 'integer', 'exists:objectifs_operationnels,id', $uniqueObjectif],
            ...
            'actions.*.rmo_ids' => ['required_with:actions', 'array', 'min:1'],
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator): void {
            ...
            if ($pao !== null && $service !== null) {
                $currentPtaId = $this->resolveCurrentPtaId();
                $existingServiceYearPta = Pta::query()
                    ...
                    ->when($currentPtaId !== null, fn ($query) => $query->whereKeyNot($currentPtaId))
                    ->exists();

                if ($existingServiceYearPta) {
                    $validator->errors()->add(
                        'service_id',
                        'Un PTA existe deja pour ce service et cette annee. Modifiez le PTA existant pour ajouter ou ajuster les actions.'
                    );
                }
            }
            ...
            foreach ($actions as $index => $actionPayload) {
                ...
                // Workflow V2 : somme des poids = 100 % si action composée avec poids.
                ...
                if ($financementRequis) {
                    ...
                    if (! $this->hasFile("actions.{$index}.justificatif_financement") && ! $this->hasExistingFinancingJustificatif($actionPayload)) {
                        $validator->errors()->add(
    ...
    {
        $actionId = is_numeric($actionPayload['id'] ?? null) ? (int) $actionPayload['id'] : 0;
        if ($actionId <= 0) {
            return false;
        }
    ...
    private function normalizeActionsInput(array $actions): array
    {
        return collect($actions)
            ...
            ->map(function (array $action): array {
                ...
                // Workflow V2 : type_action pilote, mappé vers mode_evaluation.
                $typeActionMapToMode = [
