...
| IA | `laravel/ai` 0.8.1, config `AI_DEFAULT_PROVIDER`, conversations IA persistables, module IA PTA contrôlé par validation humaine, paramétrage Q/NQ/M proposé avant import, base documentaire/corrections humaines indexables |
...
| Tests | PHPUnit 12 — **465 tests passants, 3 skipped, 2799 assertions** après paramétrage IA Q/NQ/M des actions PTA et export `IMPORT_GLOBAL` mono-feuille |
...
- **IA sous contrôle humain** : le module PTA stocke les lots analysés, normalise les lignes, propose `type_action` (`Q`, `NQ`, `M`), seuils, sous-actions et risques, expose les erreurs, impose validation/correction avant import et historise les actions IA dans `ai_import_audits`.
