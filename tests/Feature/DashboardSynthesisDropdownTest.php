...
class DashboardSynthesisDropdownTest extends TestCase
{
    ...
    public function test_dashboard_synthesis_view_exposes_decision_filters_and_advanced_tab(): void
    {
        ...
        $script = (string) file_get_contents(resource_path('js/dashboard-render.js'));
        $visualPolish = (string) file_get_contents(resource_path('js/visual-polish.js'));
        $uiEnhancements = (string) file_get_contents(resource_path('js/ui-enhancements.js'));

        $this->assertStringContainsString('data-dashboard-synthesis-filter-form', $view);
        $this->assertStringContainsString('data-responsibles-url="{{ route(\'synthese.responsibles\') }}"', $view);
        $this->assertStringNotContainsString('data-users-url=', $view);
        $this->assertStringContainsString('entry.label', $script);
        $this->assertStringContainsString('action="{{ route(\'dashboard\') }}"', $view);
        $this->assertStringNotContainsString('action="{{ route(\'synthese.index\') }}"', $view);
        $this->assertStringContainsString('name="periode"', $view);
        $this->assertStringContainsString('$synthesisPeriodOptions', $view);
        $this->assertStringContainsString('name="statut_action"', $view);
        ...
        $pilotagePosition = strpos($view, "'overview' => 'Pilotage'");
        $tablesPosition = strpos($view, "'advanced' => 'Tableaux'");
        $chartsPosition = strpos($view, "'charts' => 'Graphiques'");

        $this->assertIsInt($pilotagePosition);
        $this->assertIsInt($tablesPosition);
        $this->assertIsInt($chartsPosition);
        $this->assertLessThan($tablesPosition, $pilotagePosition);
        $this->assertLessThan($chartsPosition, $tablesPosition);
        $this->assertStringNotContainsString('\'overview\' => \'Synthèse\'', $view);
        $this->assertStringNotContainsString('\'overview\' => \'Synthese\'', $view);
        $this->assertStringNotContainsString('Vue détaillée', $view);
        $this->assertStringContainsString('data-dashboard-panel="advanced"', $tables);
        $this->assertStringContainsString('Aucune donnée pour les tableaux', $tables);
        $this->assertStringContainsString("const panelKeys = ['overview', 'advanced', 'charts'];", $script);
        $this->assertStringContainsString('$baseSynthesisQuery', $overview);
        $this->assertStringContainsString("'statut_action' => \$synthesisFilters['statut_action'] ?? null", $overview);
        $this->assertStringContainsString("return route('dashboard', array_merge(", $overview);
        $this->assertStringContainsString("['dashboardTab' => 'advanced']", $overview);
        ...
        $this->assertStringContainsString('Tableaux de pilotage', $detailTables);
        ...
        $this->assertStringContainsString('data-progressive-accordion-group', $detailTables);
        $this->assertStringContainsString('data-progressive-accordion-item', $detailTables);
        $this->assertStringContainsString('data-progressive-accordion-group', $hierarchy);
        $this->assertStringContainsString("group.querySelectorAll(':scope > details[data-progressive-accordion-item]')", $visualPolish);
        $this->assertStringContainsString('otherItem.open = false;', $visualPolish);
        $this->assertStringContainsString('if (section instanceof HTMLDetailsElement) return;', $uiEnhancements);
